...
```bash
trashtunes$ sam build --use-container
```
...
```bash
trashtunes$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
trashtunes$ sam local start-api
trashtunes$ curl http://localhost:3000/
```
...
```bash
trashtunes$ sam logs -n HelloWorldFunction --stack-name trashtunes --tail
```
...
```bash
trashtunes$ pip install pytest pytest-mock --user
trashtunes$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name trashtunes
```
